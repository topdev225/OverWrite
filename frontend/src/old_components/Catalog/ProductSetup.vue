<template>
  <v-layout justify-center column fill-height mt-3 ml-3>
    ...
    <v-flex xs10 mt-5 v-if="!showProduct">
      <v-card>
        ...
        <v-card-text>
          <v-form v-model="validProductSetup">
            <h2>{{products.name}}</h2>
            <v-btn flat color="primary" @click="$router.push('/edit_images/'+ products.id)">Edit Images</v-btn>
              ...
              <v-layout>
                ...
                <v-flex ml-2>
                  <v-card v-for="(item, index) in decorations" :key="item.index">
                    <v-text-field v-model="item.decorator_name" label="Decorator name" ></v-text-field>
                    ...
                    <v-text-field v-model.number="item.decoration_cost"
                                  ...
                                  label="Decoration Cost $" type="number" ></v-text-field>
...
<script>
  ...
  import ProductAddImport from './ProductAddImport.vue'
    ...
    methods: {
      ...
      save () {
        ...
        axios
          ...
          .then(resp => {
            this.$data.newProduct = resp.data
            this.$data.showProduct = false
            this.getProductAttributes(resp.data.product_type_id)
          })
          .catch(err => {
            console.log(err)
          })
